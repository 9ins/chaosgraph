...
public class DefaultGraphFactory {
	...
	public static <V extends Number, X, Y> AbstractGraph<V, X, Y> createGraph(GRAPH graphType, GraphElements<V, X, Y> elements, int width, int height) {
		AbstractGraph<V, X, Y> graph;
        if(graphType.compareTo(GRAPH.AREA) == 0) {
        	graph = new AreaGraph<V, X, Y>(elements, width, height);
        } else if(graphType.compareTo(GRAPH.BAR) == 0) {
        	graph = new BarGraph<V, X, Y>(elements, width, height);
        } else if(graphType.compareTo(GRAPH.BAR_RATIO) == 0) {
        	graph = new BarRatioGraph<V, X, Y>(elements, width, height);
        } else if(graphType.compareTo(GRAPH.CIRCLE) == 0) {
        	graph = new CircleGraph<V, X, Y>(elements, width, height);
        } else if(graphType.compareTo(GRAPH.LINE) == 0) {
        	graph = new LineGraph<V, X, Y>(elements, width, height);
        } else {
